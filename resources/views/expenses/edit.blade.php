        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form id="add-expense-form" method="POST" action="{{ route('expenses.update', $expense->id) }}"> {{-- Correct action URL --}}
                        @csrf
                        @method('PUT') {{-- Important for updates --}}

                        @if ($expense) {{-- Check if $expense exists (CRUCIAL) --}}
                        <input type="hidden" name="id" value="{{ $expense->id }}">

                        <div class="mb-4">
                            <x-input-label for="amount" :value="__('Amount')" />
                            <x-text-input id="amount" name="amount" type="number" class="mt-1 w-full" required autofocus value="{{ $expense->amount }}" />
                            <div id="amount-error" class="text-red-600"></div>
                        </div>

                        <div class="mb-4">
                            <x-input-label for="description" :value="__('Description')" />
                            <x-text-input id="description" name="description" type="text" class="mt-1 w-full" required value="{{ $expense->description }}" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="expense_date" name="expense_date" type="date" class="mt-1 w-full" required value="{{ $expense->expense_date }}" />
                        ...
                        @else {{-- Important: Handle the case where $expense is NOT found --}}
                        <p>Expense not found.</p> {{-- Or redirect, or show a more informative error --}}
                        @endif {{-- Always close the @if --}}
    ...
    <x-slot name="scripts">
        ...
        <script>
            $(document).ready(function() {
                const expenseForm = new window.ExpenseForm('#add-expense-form', true, {
                    id: "{{ $expense ? $expense->id : null }}" // Handle potential null $expense
                });
            });
        </script>
    </x-slot>
